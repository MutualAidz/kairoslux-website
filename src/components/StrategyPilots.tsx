...
type Step = {
  id: "discern-clarify" | "design-decide" | "mission-impact-work";
...
export function StrategyPilots() {
  ...
  // Hash-aware navigation: if the URL hash matches a step, select it.
  useEffect(() => {
    const applyHash = () => {
      const raw = window.location.hash.replace("#", "") as Step["id"] | "";
      if (!raw) return;
